...
function MetricsPanel({ subsystems }) {
  ...
  const [chartReady, setChartReady] = useState({ cpu: false, memory: false });
  ...
  useEffect(() => {
    ...
    const updateReady = () => {
      setChartReady({
        cpu: cpuContainer.clientWidth > 0 && cpuContainer.clientHeight > 0,
        memory: memoryContainer.clientWidth > 0 && memoryContainer.clientHeight > 0,
      });
    };
    ...
    if (typeof ResizeObserver !== 'undefined') {
      ...
      observer.observe(cpuContainer);
      observer.observe(memoryContainer);
     ...
     <section id="metrics" className="space-y-6 rounded-xl bg-white p-8 border border-slate-200">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className={`rounded-lg bg-slate-50 p-6 border border-slate-200 ${activeHistory.cpu[activeHistory.cpu.length - 1] > 80 ? 'border-l-4 border-l-red-600' : ''}`}>
          ...
          <div id="cpu-chart-container" className="mt-4 h-64 min-w-0">
            {chartReady.cpu ? (
        ...
        <div className={`rounded-lg bg-slate-50 p-6 border border-slate-200 ${activeHistory.memory[activeHistory.memory.length - 1] > 4000 ? 'border-l-4 border-l-red-600' : ''}`}>
          ...
          <div id="memory-chart-container" className="mt-4 h-64 min-w-0">
            {chartReady.memory ? (
              <ResponsiveContainer width="100%" height="100%">
